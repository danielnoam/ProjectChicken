...
namespace Editor
{
    [CustomPropertyDrawer(typeof(HitscanBehaviorBase), true)]
    public class HitscanBehaviorDrawer : PropertyDrawer {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
            ...
            if (EditorGUI.DropdownButton(typeRect, new GUIContent(displayName ?? "Select Behavior Type"), FocusType.Keyboard)) {
                ...
                if (_typeMap == null || _typeMap.Count == 0) {
                    menu.AddDisabledItem(new GUIContent("No Hitscan Behaviors available"));
                    menu.ShowAsContext();
                    return;
                }
        ...
        private static void BuildTypeMap() {
            var baseType = typeof(HitscanBehaviorBase);
